...
if Config.ENVIRONMENT == "production":
    # Production origins
    origins = [
        "https://word-game-frontend.onrender.com",
...
def generate_username():
    ...
    return (
        f"{random.choice(adjectives)}{random.choice(animals)}{random.randint(1, 999)}"
    )
...
@app.get("/session", response_model=GameSession)
def get_or_create_session(
    ...
    response.set_cookie(
        ...
        samesite="none",
        secure=Config.ENVIRONMENT != "development",  # Only use secure in production
...
def start_game(
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            distractors = [
                d for d in row["distractors"] if d
            ...
            steps.append(
                GameStep(
                    position=position, correct_word=correct_word, options=all_words
                )
            )
...
def make_guess(
    ...
    try:
        ...
        cursor.execute("SELECT id FROM word_pairs WHERE id = %s", (guess.pair_id,))
        ...
        if not result:
            raise HTTPException(
                status_code=404, detail="Bridge word position not found"
            )
        ...
        if updated_token:
            ...
            response.set_cookie(
                ...
                != "development",  # Only use secure in production
                domain=None,  # Let browser determine domain
            )
